...
function verifyFileType() {
    var fname = id('myfile').value.toLowerCase();
    var ftype = fname.substr(fname.lastIndexOf('.'));
    var atype = id('myfile').accept.split(",");
    var fsize = id('myfile').files[0].size;
    var max_size = Number(id('myfile').max);
...
function uploadComplete(evt) {
    var p = evt.target.responseText.search("stopUpload");
    var f = evt.target.responseText.indexOf("\"");
    var l = evt.target.responseText.lastIndexOf("\"");
